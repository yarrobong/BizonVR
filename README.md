...
- гостевой заказ после оформления получает защищённый токен доступа к странице заказа и к странице оплаты;
...
- основной публичный entrypoint реализован через `email + код`;
- альтернативный публичный entrypoint реализован через телефон и SMS-код;
- вход по email и паролю остаётся дополнительным сценарием для уже настроенных аккаунтов;
- при успешной проверке SMS-кода пользователь создаётся автоматически, если аккаунта ещё не было;
- при успешной проверке email-кода пользователь создаётся автоматически, если аккаунта ещё не было;
...
- email можно добавить и подтвердить позже в личном кабинете, если пользователь сначала вошёл по телефону.
...
- пользователь может войти по email-коду или по SMS-коду;
- если аккаунта не было, он создаётся автоматически после успешной проверки кода;
...
- после подтверждения email или телефона guest order автоматически привязывается к подходящему аккаунту;
...
4. Заполняет контакты, доставку, получателя, способ оплаты, комментарий и подтверждает юридические согласия.
5. Система создаёт заказ и переводит пользователя на страницу заказа или на инструкции по оплате.
6. Если `TEST_ORDER_NO_PAYMENT=False`, пользователь использует выбранный способ оплаты: СБП, банковскую карту или согласование с менеджером для юрлиц.
7. После подтверждения платежа заказ помечается как оплаченный.
8. Гость может открыть заказ по защищённой ссылке и сохранить его в кабинете через email из заказа.
9. Пользователь отслеживает заказ в личном кабинете в разделе истории заказов.

## Авторизация и безопасность

### Способы входа

- Email + код: основной и публично доступный способ входа.
- Телефон + SMS-код: альтернативный публичный способ входа.
- Email + пароль: дополнительный сценарий для уже настроенных аккаунтов.
...
- пароль не нужен для базового входа по email-коду или по SMS;
