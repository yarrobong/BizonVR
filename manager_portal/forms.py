...
class ManualOrderItemForm(StyledFormMixin, forms.Form):
    ...
    def has_item_data(self):
        ...
        return any(
            data.get(key)
            for key in ('product', 'variant', 'configuration', 'quantity', 'purchase_price', 'sale_price', 'discount_amount', 'comment')
        )

    def clean(self):
        cleaned = super().clean()
        if not any(cleaned.get(key) for key in ('product', 'variant', 'configuration', 'quantity', 'purchase_price', 'sale_price', 'discount_amount', 'comment')):
            return cleaned
